---
title: "Dynamic Survival Regression models via Ordinary Differential Equations. Ipilimumab data"
...
# Dynamic Survival Regression models

This document provides codes and information about the regression models based on the *Logistic ODE* hazard model proposed in @christen:2023. 

## The Logistic ODE hazard model 

The logistic ODE hazard model is defined by  [@christen:2023]

$$
\begin{cases}
h'(t)  =  \lambda h(t) \left(1 - \dfrac{h(t)}{\kappa}\right), & h(0) = h_0\\
H'(t)  =  h(t), & H(0) = 0.
\end{cases}
$$
where $\lambda > 0$ represents the intrinsic growth rate of the hazard function, $\kappa > 0$ represents the upper bound of the hazard function, and $h_0 > 0$ is the value of the hazard function at $t=0$. This ODE has the following analytic solution
$$
h(t \mid \lambda, \kappa, h_0) = \frac{\kappa h_0 e^{\lambda t}}{\kappa + h_0 (e^{\lambda t} - 1)} .
$$

## Dynamic regression model

We consider modelling the regression parameters using transformed linear predictors of the type 

$$\lambda_i = \exp( \alpha_0 + \alpha_1 \times \text{trt}_i),$$
and
$$\kappa_i = \exp( \beta_0 + \beta_1 \times \text{trt}_i).$$
We assume the initial conditions $h_0$ to be common to all individuals and unknown.


The following R code analyses a Phase 3 trial of immunotherapy to treat advanced prostate cancer (NCT00861614). We model the parameters $\lambda$ and $\kappa$ using a transformed linear predictor, both containing an intercept and a binary variable indicating the use of `treatment` (Ipilimumab) or `placebo`. See @kwon:2014 and @plana:2022 for more information.
...
# Bayesian Analysis: Logistic ODE
...
## Credible intervals: first 5 months
